        ...
        <el-col :span="6" v-for="movie in movieList" :key="movie.id">
          <el-card class="movie-card" @click="goToMovieDetail(movie.id)">
            ...
            <div class="card-body">
              ...
              <p style="font-size: 12px; color: #666; margin-bottom: 5px;">上映时间：{{ movie.release_date }}</p>
              <div style="display: flex; align-items: center; color: #ff9f43;">
                <el-icon><StarFilled /></el-icon>
                <span style="margin-left: 5px;">{{ movie.avg_score }} ({{ movie.rating_count }}人评分)</span>
              </div>
...
import { ElMessage, ElEmpty, ElPagination, ElPageHeader } from 'element-plus'
